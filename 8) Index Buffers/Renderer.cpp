...
Renderer::Renderer(Window& parent) : OGLRenderer(parent) { 
	...
	terrainTex = SOIL_load_OGL_texture(TEXTUREDIR"BarrenReds.JPG", 
		SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_MIPMAPS); 

	rockTex = SOIL_load_OGL_texture(TEXTUREDIR"Rock_04_DIFF.png",
		SOIL_LOAD_AUTO, SOIL_CREATE_NEW_ID, SOIL_FLAG_MIPMAPS);
	
	if (!terrainTex|| !rockTex) { 
		return; 
	}
	...
	projMatrix = Matrix4::Perspective(1.0f, 10000.0f, 
		(float)width / (float)height, 50.0f); 
